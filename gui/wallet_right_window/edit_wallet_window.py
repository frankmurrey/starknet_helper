...
class EmptyUiWalletData:
    ...
    private_key = ""
    pair_address = ""
    proxy = ProxyData(host="", port="")
    type = enums.PrivateKeyType.argent
    cairo_version = 1
    address = ""


class EditWalletFrame(customtkinter.CTkFrame):
    def __init__(
            self,
            master,
            wallet_data: WalletData = None,
            on_edit_wallet: Callable[[Union[WalletData, None]], None] = None,
            **kwargs,
    ):
        ...
        self.on_edit_wallet = on_edit_wallet
        ...
        self.proxy_entry = objects.CTkEntryWithLabel(
            self,
            label_text="Proxy",
            textvariable=tkinter.StringVar(value=self.wallet_data.proxy.to_string()),
        )
...
class EditWalletWindow(customtkinter.CTkToplevel):
    def __init__(
            self,
            master,
            wallet_data: WalletData = None,
            on_add_wallet: Callable[[WalletData], None] = None
    ):
        ...
        self.frame = EditWalletFrame(
            self,
            on_add_wallet=on_add_wallet,
