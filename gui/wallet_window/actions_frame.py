...
class ActionsFrame(customtkinter.CTkFrame):
    def __init__(self, master: any, **kwargs):
        ...
        self.run_settings_label.grid(
            row=0, column=1, padx=20, pady=(10, 0), sticky="ws"
        )
...
class TableTopFrame(customtkinter.CTkFrame):
    def __init__(self, master: any, **kwargs):
        ...
        self.action_name_label = customtkinter.CTkLabel(
            self, text="Module", font=customtkinter.CTkFont(size=12, weight="bold")
        )
        self.action_name_label.grid(row=0, column=0, padx=40, pady=0)

        self.action_type_label = customtkinter.CTkLabel(
            self, text="Action", font=customtkinter.CTkFont(size=12, weight="bold")
        )

        self.action_type_label.grid(row=0, column=1, padx=40, pady=0)

        self.repeats_label = customtkinter.CTkLabel(
            self, text="Repeats", font=customtkinter.CTkFont(size=12, weight="bold")
        )
        self.repeats_label.grid(row=0, column=2, padx=40, pady=0)
...
class CurrentActionsFrame(customtkinter.CTkScrollableFrame):
    def __init__(self, master: any, **kwargs):
        ...
        self.grid(row=1, column=0, padx=20, pady=(10, 0), sticky="nsew")
        self.grid_columnconfigure(0, weight=1)

        self.no_actions_label = customtkinter.CTkLabel(
            self, text="No actions", font=customtkinter.CTkFont(size=15, weight="bold")
        )
        self.no_actions_label.grid(row=0, column=0, padx=40, pady=10)
...
class ButtonActionsFrame(customtkinter.CTkFrame):
    def __init__(self, parent: any, **kwargs):
        ...
        self.grid(row=2, column=0, padx=20, pady=(5, 20), sticky="nsew")
        ...
        self.add_action_button.grid(
            row=0, column=0, padx=(30, 0), pady=(10, 10), sticky="wn"
        )

        self.add_action_label = customtkinter.CTkLabel(
            self, text="Add action", font=customtkinter.CTkFont(size=13, weight="bold")
        )
        self.add_action_label.grid(
            row=0, column=1, padx=(6, 0), pady=(10, 10), sticky="wn"
        )
        ...
        self.remove_all_actions_button.grid(
            row=0, column=2, padx=(20, 0), pady=(10, 10), sticky="ew"
        )

        self.remove_all_actions_label = customtkinter.CTkLabel(
            self, text="Clear all", font=customtkinter.CTkFont(size=13, weight="bold")
        )
        self.remove_all_actions_label.grid(
            row=0, column=3, padx=(6, 0), pady=(10, 10), sticky="w"
        )
        ...
        self.randomize_actions_checkbox.grid(
            row=0, column=4, padx=(20, 0), pady=(10, 10), sticky="ew"
        )

    def add_action_button_event(self):
        if (
            self.actions_top_level_window is None
            or not self.actions_top_level_window.winfo_exists()
        ):
            self.actions_top_level_window = InteractionTopLevelWindow(
                parent=self.master
            )
            self.actions_top_level_window.geometry("600x1000+1385+100")
        else:
...
class RunSettingsFrame(customtkinter.CTkFrame):
    def __init__(self, parent: any, **kwargs):
        ...
        self.grid(row=1, column=1, padx=20, pady=(10, 20), sticky="nsew", rowspan=2)
        ...
        self.test_mode_checkbox.grid(row=0, column=0, padx=20, pady=20, sticky="ew")
        ...
        self.min_delay_label.grid(row=1, column=0, padx=20, pady=0, sticky="w")

        self.min_delay_entry_spinbox = FloatSpinbox(self, step_size=5, width=105)
        self.min_delay_entry_spinbox.entry.configure(textvariable=Variable(value=40))
        self.min_delay_entry_spinbox.grid(
            row=2, column=0, padx=20, pady=(0, 10), sticky="w"
        )
        ...
        self.max_delay_label.grid(row=1, column=1, padx=20, pady=0, sticky="w")

        self.max_delay_entry_spinbox = FloatSpinbox(self, step_size=5, width=105)
        self.max_delay_entry_spinbox.entry.configure(textvariable=Variable(value=80))
        self.max_delay_entry_spinbox.grid(
            row=2, column=1, padx=20, pady=(0, 10), sticky="w"
        )
    ...
    def wait_for_txn_checkbox_event(self):
        if self.wait_for_receipt_checkbox.get():
            self.txn_wait_timeout_seconds_spinbox.entry.configure(
                state="normal", fg_color="gray16", textvariable=Variable(value=120)
            )
            self.txn_wait_timeout_seconds_spinbox.add_button.configure(state="normal")
    ...
    def test_mode_checkbox_event(self):
        """
        Toggle checkbox text color if test mode is enabled
        Returns: None
        """
        if self.test_mode_checkbox.get():
            self.test_mode_checkbox.configure(text_color="#6fc276")
        else:
            self.test_mode_checkbox.configure(text_color="#F47174")
